...
const links = [
	...
	{
		name: 'Resources',
		href: 'https://blog.abcfoundationconnect.com/',
		submenu: true,
		submenuItems: [{ name: 'Articles', href: 'https://blog.abcfoundationconnect.com/' }],
	},
		...
		<header className="border-b bg-white py-5 dark:border-neutral-800 dark:bg-neutral-900">
			<div>
								...
								<Link href={listData.href} className="" key={idx}>
									{listData.submenu ? (
										<div className="group relative">
											<h3>{listData.name}</h3>
											<div className="absolute z-[999] hidden w-[200px] rounded-lg border p-3 shadow-lg group-hover:block">
												{listData.submenuItems.map((data, idx) => (
													<h6 className="text-sm" key="idx">
														{data.name}
													</h6>
												))}
											</div>
				...
				<div className={`overflow-hidden lg:hidden ${isSidebarVisible ? 'h-fit' : 'h-0'}`}>
					<div
						className={` flex flex-col space-y-6 py-10 pl-8 text-lg 
				transition ${isSidebarVisible ? 'translate-y-0' : '-translate-y-96'}`}
					>
							...
							<Link href={listData.href} key={idx}>
								{listData.submenu ? (
									<div className="group">
										<h3>{listData.name}</h3>
										{listData.submenuItems.map((data, idx) => (
											<h6 className="hidden text-xl  group-hover:block" key="idx">
												{data.name}
											</h6>
										))}
									</div>
						...
						<Link
							href="https://donate.abcfoundationconnect.com/b/8wMaEK1aw8OGdj2144"
							target="_blank"
							className="bg-abcf h-fit w-fit rounded-lg px-6 
					py-3 text-sm
